...
# O `cd "$TEMP_DIR"` mais abaixo faz o node gravar relativo ao temp, que é apagado no fim:
# caminho relativo aqui = "sucesso" impresso e nenhum arquivo. Medido em 21/08/2026.
...
// Congela no estado FINAL: desliga transitions/animations para o screenshot capturar
// o conteúdo já revelado (independe do tempo da animação de entrada do deck).
await page.addStyleTag({ content: '*{transition:none !important;animation:none !important;}' });

// Extra wait for fonts/layout to settle on the first slide
...
# Nunca anunciar sucesso sem o arquivo na mão (anti-proxy: exit 0 não é evidência).
if [ ! -s "$OUTPUT_PDF" ]; then
  echo ""
  echo -e "${BOLD}✗ FALHOU:${NC} o PDF não foi gravado em $OUTPUT_PDF" >&2
  exit 1
fi
...
# Abrir o PDF é conveniência, não o resultado: em servidor sem display o xdg-open
# falha e o exit code dele vira o do script — quem checa $? conclui falha numa
# exportação que deu certo. Silenciar e sair 0 explicitamente. (21/08/2026)
if command -v open &>/dev/null; then
    open "$OUTPUT_PDF" >/dev/null 2>&1 || true
