...
Ponte **HTML (tema Elegant/reMarkable) → PPTX editável**. O irmão `/elegant-html-slides` é ótimo para prototipar e revisar macro; **mas editar HTML na mão é contraprodutivo**. Quando o Flavio vai "meter a mão na massa", ele para num PowerPoint. Esta skill faz essa passagem preservando a identidade P&B.
...
Foi assim que nasceram o W2 RIPack v04 (`03_Scripts/v04/lib.py`, clonando do W1 v42) e o deck do post da
sabatina (`lib/exemplo_build_regua_w2.py`): `clone()` copia o `spTree` e religa as imagens, `set_text()`
...
Regra do Flavio para qualquer slide (05/09/2026): **nunca um slide chato; o design representa a ideia.**
...
- **SVG inteiro do slide → DrawingML** (ppt-master, ENG-266) → fiel, mas vira mosaico de vetores que ninguém edita no PowerPoint. NÃO.
...
- `/mondore_pptx` — de onde vem a LÓGICA (construção nativa, INTENT-first, coherence pass, rubrica). NÃO a identidade Mondoré (esta skill é neutra P&B).
- ppt-master (ENG-266) — pipeline SVG→pptx, aproveitado só pros glifos decorativos.
...
### 2. Fontes (uma vez)
Literata + Inter são Google Fonts (OFL, embutíveis). lib/instance_fonts.py baixa as variable e gera 7 faces estáticas (Literata Light R/I, Literata R/I, Inter R/B, Inter SemiBold). Instale-as (~/.fonts + fc-cache) para o render fiel via LibreOffice. Já vêm prontas em fonts/.
...
pede) com o mesmo `instance_fonts.py`.
...
## Feedbacks do Flavio incorporados (revisão v3→v4, 22/07)
...
- **DEFAULT: NÃO embutir fontes.** Entregar o pptx LIMPO + o zip das fontes pra instalar (`RIPack_fontes_tema.zip`). Fonte instalada > embed inverificável.
