...
- **Iterative** (`iterative <arquivo>`): documento inteiro com ≥10 claims factuais, **Workflow multi-round com agentes paralelos**. Validado em entregas acadêmicas MPA (BIZ-190 Supply Chain Di Serio, BIZ-191 ESG).
...
5. **Reportar a Flavio** com 3 níveis:
...
- **Quote no blog ≠ quote em artigo de terceiros.** Se a frase aparece num artigo Medium, é o jornalista falando. Atribuir corretamente. Caso real: BIZ-172, frase atribuída ao Steinberger no início → era do artigo Global Times Singapore.
...
- **Limitar tokens do Haiku.** Spawn pequeno — máximo 250 palavras de resposta esperada. Se precisar de pesquisa ampla, escalar para Sonnet via Birdperson.
...
Invocado por `/writing fact-check iterative <arquivo.docx|.md>`. Para entregas com ≥10 claims factuais — entrega final MPA, ensaio individual, longform com numerosas atribuições. Validado em BIZ-190 (Supply Chain Di Serio, v1→v10, 6 rounds, 22+ agentes verificadores, zero erros críticos confirmados na entrega).
...
### Regras críticas (anti-padrões observados em BIZ-190)
...
# Roda só round 3 (sanity textual) — útil em iteração final após Flavio aprovar conteúdo
```
...
- Tarefa onde a "fonte primária" é o próprio Flavio (memória, opinião) — não tem como verificar contra documento externo
