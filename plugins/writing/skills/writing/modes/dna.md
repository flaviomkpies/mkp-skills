...
4. Ao lado de cada regra inviolável recente, mostrar a fonte (ex: "deployed 2026-04-21, BIZ-168").

## Procedimento (edit)

1. **AskUserQuestion** sobre tipo de mudança:
   - **Adicionar nova regra** — Flavio descreve. Perguntar: o que motivou (incidente, padrão recorrente, leitura externa)? É genérica (DNA central) ou específica de um modo (ex: só newsletter)?
   - **Modificar regra existente** — Flavio aponta qual. Perguntar: o que mudou de contexto? A regra antiga continua válida em outro escopo?
   - **Remover regra** — Flavio aponta qual. Perguntar: por quê? Há registro do motivo original?
...
3. **Sumário da mudança** — produzir um diff curto da edição para Flavio aprovar antes de salvar.
4. **Salvar.** Atualizar `DNA.md` (ou modo específico) com timestamp.
5. **Memory:** se a mudança veio de incidente recorrente (3+ correções iguais), salvar em memory persistente do Birdperson para referência futura.

## Critérios de promoção (quando uma correção vira regra)

Hoje, regras entram no DNA quando:
- Padrão de correção do Flavio repete **3 vezes** em posts diferentes.
- Incidente único MAS de impacto editorial alto (atribuição errada, fact-check furado).
- Aprendizado externo (livro, post de Paul Graham/Naval/Mollick) que Flavio quer adotar.
